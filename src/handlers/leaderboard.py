...
def _kb(active: str):
    ...
    for label, key in _TABS:
        b.button(
            ...
            callback_data=LeaderboardCb(action="show", board=key).pack(),
        )
    b.button(text="✖ Chiudi", callback_data=LeaderboardCb(action="close").pack())
    b.adjust(3, 1)
    return b.as_markup()
...
@router.callback_query(LeaderboardCb.filter(F.action == "show"))
async def cb_lead(
    callback: CallbackQuery, callback_data: LeaderboardCb, db_session: AsyncSession
) -> None:
    board = callback_data.board
...
@router.callback_query(LeaderboardCb.filter(F.action == "close"))
async def cb_lead_close(callback: CallbackQuery) -> None:
    try:
